...
class MusicCog(commands.Cog):
    ...
    async def handle_error(self, interaction: Interaction, error: Exception, *callbacks: Callable[[], None]) -> None:
    ...
    async def handle_play_track_now_error(self, interaction: Interaction, error: Exception):
        await self.handle_error(
            interaction, 
            error, 
            lambda: update_guild_states(self.guild_states, interaction, (False, False, False), ("is_modifying", "is_extracting", "voice_client_locked")),
            lambda: update_query_extraction_state(self.guild_states, interaction, 0, 0, None, None)
        )
    ...
    async def skip_track(self, interaction: Interaction, amount: int=1):
        ...
        update_guild_states(self.guild_states, interaction, (True, True), ("is_modifying", "user_interrupted_playback"))

        skipped = skip_tracks_in_queue(queue, current_track, is_looping, amount)
        if isinstance(skipped, Error):
            update_guild_states(self.guild_states, interaction, (False, False), ("is_modifying", "user_interrupted_playback"))
    ...
    async def show_next_track(self, interaction: Interaction):
        ...
        next_track = get_next_visual_track(is_random, is_looping, track_to_loop, filters, queue, queue_to_loop)
    ...
    async def show_previous_track(self, interaction: Interaction):
        ...
        previous = get_previous_visual_track(current_track, history)
    ...
    async def pause_track(self, interaction: Interaction):
        ...
        if can_update_status:
            update_guild_state(self.guild_states, interaction, f"Listening to '{current_track['title']}' (paused)", "voice_status")
    ...
    async def handle_pause_track_error(self, interaction: Interaction, error: Exception):
        await self.handle_error(
            interaction, 
            error, 
            lambda: update_guild_state(self.guild_states, interaction, False, "voice_client_locked")
        )
    ...
    async def resume_track(self, interaction: Interaction):
        ...
        if can_update_status:
            update_guild_state(self.guild_states, interaction, f"Listening to '{current_track['title']}'", "voice_status")
            await set_voice_status(self.guild_states, interaction)
    ...
    async def handle_stop_track_error(self, interaction: Interaction, error: Exception):
        await self.handle_error(
            interaction, 
            error, 
            lambda: update_guild_states(self.guild_states, interaction, (False, False), ("voice_client_locked", "stop_flag"))
        )
    ...
    async def select_track(self, interaction: Interaction, track_name: str, by_index: bool=False, keep_current_track: bool=False):
        ...
        found = find_track(track_name, queue, by_index)
        if isinstance(found, Error):
            update_guild_state(self.guild_states, interaction, False, "is_modifying")
        ...
        update_guild_states(self.guild_states, interaction, (False, True), ("is_modifying", "voice_client_locked"))
        if voice_client.is_playing() or voice_client.is_paused():
            update_guild_state(self.guild_states, interaction, True, "stop_flag")
    ...
    async def handle_select_track_error(self, interaction: Interaction, error: Exception):
        await self.handle_error(
            interaction, 
            error, 
            lambda: update_guild_states(self.guild_states, interaction, (False, False, False), ("is_modifying", "voice_client_locked", "stop_flag"))
        )
    ...
    async def select_random_track(self, interaction: Interaction, keep_current_track: bool=False):
        ...
        update_guild_states(self.guild_states, interaction, (True, True), ("is_modifying", "voice_client_locked"))
        ...
        update_guild_states(self.guild_states, interaction, (False, False), ("is_modifying", "voice_client_locked"))
    ...
    async def replace_track(self, interaction: Interaction, old_track_name: str, new_track_query: str, search_provider: app_commands.Choice[str]=None, by_index: bool=False):
        ...
        if is_looping_queue:
            update_loop_queue_replace(self.guild_states, interaction, result[1], result[0])
    ...
    async def loop_track(self, interaction: Interaction, track_name: str=None, by_index: bool=False):
        ...
        if not is_looping or (is_looping and track_name):
            if track_name is not None:
                ...
                found_track = find_track(track_name, queue, by_index)

                if isinstance(found_track, Error):
                    await interaction.followup.send(found_track.msg)
                    return
                
                update_guild_state(self.guild_states, interaction, found_track[0], "track_to_loop")
            else:
                update_guild_state(self.guild_states, interaction, current_track, "track_to_loop")
            update_guild_state(self.guild_states, interaction, True, "is_looping")
            
            await interaction.followup.send(f"Loop enabled!\nWill loop '**{self.guild_states[interaction.guild.id]['track_to_loop']['title']}**'.")
        else:
            update_guild_states(self.guild_states, interaction, (None, False), ("track_to_loop", "is_looping"))
    ...
    async def randomize_track_selection(self, interaction: Interaction):
        ...
        if not is_random:
            update_guild_state(self.guild_states, interaction, True, "is_random")
            await interaction.followup.send("Track randomization enabled!\nWill choose a random track from the queue at next playback.")
        else:
            update_guild_state(self.guild_states, interaction, False, "is_random")
    ...
    async def loop_queue(self, interaction: Interaction, include_current_track: bool=True):
        ...
        if not is_looping_queue:
            ...
            update_guild_states(self.guild_states, interaction, (True, new_queue), ("is_looping_queue", "queue_to_loop"))
            
            await interaction.followup.send(f"Queue loop enabled!\nWill loop **{len(new_queue)}** tracks at the end of the queue.")
        else:
            update_guild_state(self.guild_states, interaction, False, "is_looping_queue")
            queue_to_loop.clear()
    ...
    async def remove_track(self, interaction: Interaction, track_names: str, by_index: bool=False):
        ...
        if is_looping_queue:
            update_loop_queue_remove(self.guild_states, interaction, result)
    ...
    async def reposition_track(self, interaction: Interaction, track_name: str, new_index: int, by_index: bool=False):
        ...
        result = reposition_track_in_queue(track_name, new_index, queue, by_index)
        if isinstance(result, Error):
            update_guild_state(self.guild_states, interaction, False, "is_modifying")
    ...
    async def show_history(self, interaction: Interaction, page: int):
        if not await user_has_role(interaction) or\
            not await check_channel(self.guild_states, interaction) or\
            not await check_guild_state(self.guild_states, interaction, "queue_history", [], "Track history is empty. Nothing to view."):
            return
    ...
    async def stop_extraction(self, interaction: Interaction):
        ...
        update_guild_state(self.guild_states, interaction, False, "can_extract")
    ...
    async def apply_track_filters(self, interaction: Interaction, min_duration: str=None, max_duration: str=None, author: str=None, website: app_commands.Choice[str]=None):
        if not await user_has_role(interaction) or\
            not await check_channel(self.guild_states, interaction) or\
            not await check_guild_state(self.guild_states, interaction, "is_editing_filters", True, "Filters are already being modified, please wait.") or\
            not await check_guild_state(self.guild_states, interaction, "is_modifying", True, "The queue is currently being modified, please wait.") or\
        ...
        update_guild_state(self.guild_states, interaction, True, "is_editing_filters")
        ...
        await interaction.followup.send(
            ...
            get_added_filter_string(filters, added)
        )
    ...
    async def handle_apply_track_filters_error(self, interaction: Interaction, error: Exception):
        await self.handle_error(
            interaction, 
            error,
            lambda: update_guild_state(self.guild_states, interaction, False, "is_editing_filters")
        )
    ...
    async def clear_track_filters(self, interaction: Interaction, min_duration: bool=False, max_duration: bool=False, author: bool=False, website: bool=False):
        ...
        await interaction.followup.send(
            ...
            get_removed_filter_string(removed)
        )
    ...
    async def show_filters(self, interaction: Interaction):
        if not await user_has_role(interaction) or\
            not await check_channel(self.guild_states, interaction) or\
            not await check_guild_state(self.guild_states, interaction, "is_editing_filters", True, "Filters are being modified, please wait.") or\
            not await check_guild_state(self.guild_states, interaction, "is_modifying", True, "The queue is currently being modified, please wait.") or\
            not await check_guild_state(self.guild_states, interaction, "voice_client_locked", True, "Voice state currently locked! Wait for the other action first."):
            return
        ...
        await interaction.followup.send(
            ...
            get_active_filter_string(filters)
        )
    ...
    async def show_current_track_info(self, interaction: Interaction):
        ...
        queue_to_loop = self.guild_states[interaction.guild.id]["queue_to_loop"]
        track_to_loop = self.guild_states[interaction.guild.id]["track_to_loop"]
        queue_state_being_modified = self.guild_states[interaction.guild.id]["is_modifying"]
    ...
    async def set_allow_greetings(self, interaction: Interaction, enable: bool):
        ...
        update_guild_state(self.guild_states, interaction, enable, "allow_greetings")
    ...
    async def set_allow_voice_status_edit(self, interaction: Interaction, enable: bool):
        ...
        update_guild_states(self.guild_states, interaction, (True, enable), ("voice_client_locked", "allow_voice_status_edit"))
        ...
        if not enable and voice_status is not None:
            ...
            update_guild_state(self.guild_states, interaction, status, "voice_status")
            await set_voice_status(self.guild_states, interaction)
