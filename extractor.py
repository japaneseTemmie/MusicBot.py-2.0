...
URL_PATTERNS = [
    (re.compile(r"(?:https?:\/\/)?(?:www\.)?youtube\.com\/(?:playlist\?list=|watch\?.*?&list=)([a-zA-Z0-9_-]+)"), SourceWebsite.YOUTUBE_PLAYLIST),
    (re.compile(r"(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)/(watch\?v=|playlist\?list=|embed\/|v\/|shorts\/)?([^&=%\?]{11})"), SourceWebsite.YOUTUBE),
    (re.compile(r"(https:\/\/)?(www\.)?newgrounds\.com/audio/listen/[0-9]+\/?"), SourceWebsite.NEWGROUNDS),
    (re.compile(r"(https:\/\/)?(www\.)?soundcloud\.com/[^\/]+\/[^\/]+"), SourceWebsite.SOUNDCLOUD),
    (re.compile(r"(https:\/\/)?(www\.)?([a-z0-9\-]+)\.bandcamp\.com\/track\/[a-z0-9\-]+(\/)?"), SourceWebsite.BANDCAMP)
]
# Hashmap of search providers
# Key is the provider and the value is a tuple with yt-dlp search string [0] and 'source_website' [1]
SEARCH_PROVIDERS = {
    "soundcloud": ("scsearch:", SourceWebsite.SOUNDCLOUD_SEARCH),
    "youtube": ("ytsearch:", SourceWebsite.YOUTUBE_SEARCH)
}
...
def parse_info(info: dict, query: str, query_type: tuple[re.Pattern | str, str]) -> dict | list[dict] | Error:
    ...
    if source_website == SourceWebsite.YOUTUBE_PLAYLIST and "entries" in info:
        return [prettify_info(entry, source_website) for entry in info["entries"]]

    # If it's a search, prettify the first entry and return
    if source_website in (SourceWebsite.YOUTUBE_SEARCH, SourceWebsite.SOUNDCLOUD_SEARCH) and "entries" in info:
